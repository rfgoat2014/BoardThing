...
// ===== Actions to update the position of the cluster on the board
	...
	.exec(function(err, board) {
        if (err) {
    		...
    		if ((!board.isPrivate)||
				...
				.exec(function(err, cluster) {
					if (cluster) {
						// set and store the position of the card on the board
						...
						cluster.save(function(err) {
							if (err) {
								...
								// set the last modified time stamp for the board
					        	board.lastModified = new Date();
								board.save();
